...
public class Main {
    ...
    private static final Odds[] oddsArray = {
            new Odds(-400, 80.00),
            new Odds(+400, 20.00),
            new Odds(-395, 79.80),
            new Odds(+395, 20.20),
    ...
    static int findClosestMoneyLine(double playerOdds, Odds[] oddsArray) {
    ...
    static double calculateHitRatePercentage(double combinedProbability) {
    ...
    private static double calculateCombinedImpliedProbability(List<PlayerName> combination) {
        ...
        for (PlayerName player : combination) {
            int playerMoneyLine = findClosestMoneyLine(player.getPlayerOdds(), Main.oddsArray);
    ...
    private static double calculateCombinedProbability(List<PlayerName> combination) {
        if (combination.isEmpty()) {
            return 0.0; // If there are no players, return 0 probability
        }
...
// Iterate over players and print their information
        ...
        // Inject the oddsArray into the FantasyForgeApplication
        SwingUtilities.invokeLater(() -> {
            LoginDialog loginDlg = new LoginDialog(null);
            loginDlg.setVisible(true);
            if (loginDlg.isSucceeded()) {
                FantasyForgeApplication app = new FantasyForgeApplication(getOddsArray());
                app.show();
            } else {
                System.exit(0);
            }
        ...
        for (PlayerName player : players) {
            ...
            double combinedProbability = calculateCombinedProbability(Collections.singletonList(player));

            // Calculate combined implied probability
            double combinedImpliedProbability = calculateCombinedImpliedProbability(Collections.singletonList(player));
    ...
    // Provide access to the oddsArray
    public static Odds[] getOddsArray() {
        return oddsArray;
    }
